...
Build one with `Span.seconds`, which takes the seconds a recording documents itself in, or with
`Span.micros` when the source already has integers. Each returns the shape its bounds describe, so a
field that accepts only an interval can say so and be checked before the code runs.

```python
IntervalSpan.seconds(5.0, 8.0, time_series_ids=("vibration",))   # -> IntervalSpan, start=5_000_000
PointSpan.seconds(1.2)                                        # -> PointSpan, on every series
IntervalSpan.micros(5_000_000, 8_000_000)                        # the same interval, written directly
```
...
- `ClassificationTask`: one categorical label, for the whole sample or for `scope`; `target_schema` names
  ...
  dataset.add_task(
      ...
      scope=IntervalSpan.seconds(120.0, 480.0, time_series_ids=(vibration.time_series_id,)),
  )
  ```
...
- `TemporalLocalizationTask`: find the regions matching the prompt. `mode` is `SPARSE` (unmarked time is
  ...
  dataset.add_task(sample, TemporalLocalizationTask(
      ...
      target=(
          IntervalSpan.seconds(1.20, time_series_ids=("II",)),
          IntervalSpan.seconds(2.05, time_series_ids=("II",)),
      ),
  ))
  ```
