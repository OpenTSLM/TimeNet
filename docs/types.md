...
| `categories` | `tuple[str, ...]` | no | Ordered value-to-label codebook for `dtype="enum"`. Empty for every other dtype. |
...
read back as text. An `"enum"` dtype carries its category codebook in `categories`; its values are the
labels, and the torch bridge maps them to integer codes. Multidimensional values require the Zarr
values backend.
