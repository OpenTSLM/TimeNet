...
| `start_time` | `int \| None` | Wall-clock anchor: the Unix time (UTC, integer nanoseconds) that relative time zero refers to, for every series and annotation on the sample. `None` means no wall-clock reference exists (e.g. de-identified or synthetic data) — never fabricate one. |
...
`time_series_ids` resolve to series on the sample, and that a trial-level `TemporalAnnotation` is only
