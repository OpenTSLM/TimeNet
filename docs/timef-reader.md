...
`spec_type`; annotations are rebuilt as real `StaticAnnotation` / `TemporalAnnotation` /
`TemporalAnnotation` instances (values decoded from JSON); tasks are resolved against the built-in
