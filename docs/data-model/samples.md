...
Each modality is declared once through a typed [spec](../types.md): a name and the unit of its values,
carried as a [pint](https://pint.readthedocs.io) unit. Because the unit is part of the type, an
accelerometer trace in g, a temperature channel in °C, and a market series in a currency all read
through the same API. The spec declares no unit for time: time offsets are whole microseconds by
construction, so there is no other unit a caller could state.
