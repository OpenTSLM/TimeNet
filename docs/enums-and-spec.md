...
Four kinds of vocabulary appear across the rest of the spec:
...
- **Tasks**: frozen dataclasses (`ClassificationTask`, `LabelingTask`, `CaptioningTask`, `QATask`, `ForecastingTask`, `ReasoningTask`). The class itself names the kind of supervision and an instance carries the data for one annotation.
- **Enums**: closed `StrEnum`s that supply shared vocabularies (`Domain`, `License`, and the unit enums `SamplingRateUnit`, `TimestampUnit`, `ValueUnit`).
- **Version**: a value type that enforces semantic `major.minor.patch` versioning.

Specs are dataset-specific (each connector ships its own). Task classes, enums, and `Version` are global.
...
- An **instance** carries the data for one annotation. `Sample.annotate(ClassificationTask(label="afib"))` creates a concrete labeled annotation.
...
| `channels`  | `tuple[str, ...] \| None`                 | Channels of the parent sample the annotation targets. Must be drawn from `Sample.signals[*].channels`. `None` = whole sample. |
...
sample.annotate(ClassificationTask(label="afib"))

sample.annotate(LabelingTask(
    label="walking",
    channels=("accel_x", "accel_y", "accel_z"),
    windows_s=((120.0, 480.0),),
))

sample.annotate(QATask(
    question="What is happening between 12s and 18s?",
    answer="ST elevation in V2.",
))

target.annotate(ForecastingTask(
