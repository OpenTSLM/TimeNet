...
A spec records its `nullable` flag in the same way. A manifest written before the flag existed omits it and reads back as `False`. Every dataset built before nullability existed still loads unchanged.

Adding nullability needed no format version bump. The flag turns on only the behaviour that no older artifact used.
