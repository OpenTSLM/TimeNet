...
than one row group, which is what lets the reader skip the ones a lookup cannot match. Annotations are
sorted by their `id` column and the index by `sample_id`, the columns each is looked up on, so those
skips actually fire.
