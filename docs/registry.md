...
A registry serves compiled TimeF versions to the client. A compiled TimeF version has manifests,
a DuckDB control plane, and a values plane in Parquet or Zarr format. A registry never runs
...
!!! note "Catalog support"
    ...
    and the S3 backend reads AWS credentials from the environment. The S3 backend has no catalog,
    so it does not support `list` and `search`.

### Remote registries

`http(s)://` and `timenet://` both select `RemoteRegistry`. `timenet://` is a shorthand for the hosted
`https://registry.timenet.ai` service root. It takes no path: you pass dataset ids to the client
methods, not inside the registry URI. `RemoteRegistry` uses an `http(s)://` URI as the service root
without a change. Use that form to point at a private or a dev deployment.

The HTTP transport authenticates with a bearer token read from `$TIMENET_TOKEN`. When the token is
unset, the client reads the registry anonymously (no `Authorization` header). This is enough for
public reads. Set the token to reach private datasets or to write:
...
endpoint override from the environment (`AWS_*` vars, `AWS_PROFILE`, `AWS_ENDPOINT_URL`). Nothing is
...
`download` populated it. The backend has no catalog, so `list` and `search` do not work. Publish and
fetch with an explicit `org/name@version`.
...
| `open_version(dataset_id, version=None)` | A `DatasetVersion`: the parsed manifest plus a filesystem-rooted, picklable handle to the version's files. The [reader](timef-reader.md) reads through this handle, so a read never re-opens the registry and never re-parses `manifest.json`. |
...
| Method | Description |
| --- | --- |
| `store(dataset, *, force=False, progress_cb=None)` | Compiles a dataset, publishes it, and returns the stored version. Derives the schema first if absent. Skips an already-committed version unless `force`. |
...
Every backend is a `WritableRegistry`. As a result, this type alone does not show whether a backend
is a directory the engine can write to or a remote stub. `local_registry_path(uri)` gives this
information. It returns the directory that a `file://` URI or a plain path names. It raises
`TimeNetRegistryError` for a remote scheme.

`default_registry_path()` uses `local_registry_path` to find the default local registry. It uses
`$TIMENET_REGISTRY` when that value is a local path. Otherwise, it uses `<home>/registry`. This is
how [`timenet-build build`](cli/build.md) resolves its output when `--out` is absent. The
`timenet_connectors.builder` and `load` helpers use it too.

### Remote publishing

`RemoteRegistry.store` compiles the dataset locally, then sends the artifacts to the service over
the REST contract. Dataset bytes never pass through the API: the service issues a presigned PUT per
file and the client uploads straight to the object store. The token must be a writer token
(`$TIMENET_TOKEN`). The service rejects an anonymous or read-only caller.

1. Compile into a temporary staging directory with a [`TimeFWriter`](timef-writer.md). If the
   dataset has no schema, derive it first. An already-committed version returns early unless
   `force`.
...
3. For each file, `POST /datasets/{id}/{version}/publish/upload-url` with `{"path": relpath}`. The
   response is a presigned grant (`{"url", "headers"}`). `PUT` the file's bytes to that URL with the
   returned headers.
4. `POST /datasets/{id}/{version}/finalize` to commit the version. TimeNet removes the staging
   directory whether or not the publish succeeds.
