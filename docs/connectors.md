...
- `ensure_archive(url, target)` downloads a zip file and extracts it into `target`. This method is
  ...
  and skips the download. A successful extraction then deletes the archive: the extracted tree is
  the raw source from that point, and holding both halves the disk a build needs at its peak. An
  extraction that *fails* keeps the archive, so the retry re-extracts instead of downloading again.
...
build needs the sources only during conversion, and they often outweigh the dataset they produce
several times over. To keep them, pass `--keep-cache`, or `keep_cache=True` to
`timenet_connectors.build()`. That is worth doing while you iterate on a connector against a large
source, so each rebuild is not another full download.
