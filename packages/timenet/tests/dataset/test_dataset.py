...
def test_add_task_links_record_and_task(make_series):
    ...
    task = ds.add_task(
        task=ClassificationTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("afib",)
        )
    )
...
def test_add_task_multiple_records(make_series):
    ...
    task = ds.add_task(
        task=ClassificationTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(s1, s2), targets=("x",)
        )
    )
...
def test_from_tasks_on_constructor_registers(make_series):
    ...
    base = ds.add_task(
        task=ClassificationTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("a",)
        )
    )
    derived = ds.add_task(
        task=AnswerTask(
...
def test_add_task_rejects_a_from_tasks_parent_that_is_not_registered(make_series):
    ds = _dataset()
    record = ds.add_record(record=Record(sources=(Source(name="Source", signals=(make_series(),)),)))
    orphan = ClassificationTask(
        input_modalities=frozenset({InputModality.NO_INPUT}), targets=("a",)
    ...
    with pytest.warns(SpanOutsideWindowWarning, match="falls outside record"):
        dataset.add_task(
            task=ClassificationTask(
                ...
                inputs=(record,),
                targets=("walking",),
                scope=TimeInterval.seconds(5.0, 20.0),
            )
        )
...
def test_add_task_accepts_an_interval_up_to_the_exclusive_window_end(make_series):
    ...
    task = dataset.add_task(
        task=ClassificationTask(
            ...
            inputs=(record,),
            targets=("walking",),
            scope=TimeInterval.seconds(2.0, 10.0),
        )
    )
...
def test_add_task_accepts_scope_inside_record_span(make_series):
    ...
    task = dataset.add_task(
        task=ClassificationTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("walking",), scope=scope
        )
    )
...
def test_add_task_requires_an_answer(make_series):
    ...
    with pytest.raises(TimeFValidationError, match="needs an answer"):
        dataset.add_task(
            task=ClassificationTask(input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,))
        )
...
def test_streamed_task_validation_rejects_an_undeclared_type(make_series):
    ...
    task = ClassificationTask(input_modalities=frozenset({InputModality.TIME_SERIES}), targets=("a",), inputs=(record,))
...
def test_add_task_accepts_a_series_answer_without_a_target(make_series):
    ...
    task = dataset.add_task(
        task=ForecastingTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(context,), targets=(target,)
        ),
    )
...
def test_tasks_of_filters_by_type(make_series):
    ...
    classification = ds.add_task(
        task=ClassificationTask(input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(s,), targets=("a",))
    )
    qa = ds.add_task(
        task=AnswerTask(
            input_modalities=frozenset({InputModality.TEXT, InputModality.TIME_SERIES}),
...
def test_tasks_for_resolves_and_filters_record_tasks(make_series):
    ...
    s2 = ds.add_record(record=Record(sources=(Source(name="Source", signals=(make_series(),)),)))
    classification = ds.add_task(
        task=ClassificationTask(input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(s1,), targets=("a",))
    )
    qa = ds.add_task(
        task=AnswerTask(
            input_modalities=frozenset({InputModality.TEXT, InputModality.TIME_SERIES}),
    ...
    ds.add_task(
        task=ClassificationTask(input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(s2,), targets=("c",))
    )
...
def test_to_features_and_targets_returns_arrow_matrix_and_targets(make_series):
    ...
    for label in ["a", "b"]:
        ...
        ds.add_task(
            task=ClassificationTask(
                input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(s,), targets=(label,)
            )
        )
...
def _ragged_dataset(make_series):
    ...
    ds.add_task(
        task=ClassificationTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(first,), targets=("a",)
        )
    )
    ds.add_task(
        task=ClassificationTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(second,), targets=("b",)
        )
    )
    return ds
...
def test_to_features_and_targets_unlabeled_record_raises(make_series):
    ...
    ds.add_task(
        task=ClassificationTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(labeled,), targets=("a",)
        )
    )
    ds.add_record(
        record=Record(sources=(Source(name="Source", signals=(make_series(values=(1.0, 2.0, 3.0)),)),))
...
def test_to_features_and_targets_multiple_matching_tasks_raises(make_series):
    ...
    ds.add_task(
        task=ClassificationTask(input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(s,), targets=("b",))
    )
    with pytest.raises(TimeFValidationError, match="needs exactly one per record"):
        ds.to_features_and_targets(task=ClassificationTask)
...
def test_to_features_and_targets_rejects_a_task_with_no_inline_target(make_series):
    ...
    ds.add_task(
        task=ClassificationTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(s,), target_annotations=(annotation,)
        )
    )
...
def test_annotation_and_task_agree_on_an_out_of_window_span(make_series):
    ...
    with pytest.warns(SpanOutsideWindowWarning, match="falls outside record"):
        dataset.add_task(
            task=ClassificationTask(
                input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("x",), scope=outside
            )
        )
...
def test_add_tasks_registers_all_in_order_and_links(make_series):
    ...
    tasks = ds.add_tasks(
        tasks=[
            ...
            ClassificationTask(
                input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("b",)
            ),
        ]
    )
...
def test_add_tasks_rejects_the_whole_batch_when_one_is_invalid(make_series):
    ...
    good = ClassificationTask(input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("a",))
...
def test_add_tasks_allows_deriving_from_another_task_in_the_same_batch(make_series):
    ...
    ds = _dataset()
    record = ds.add_record(record=Record(sources=(Source(name="Source", signals=(make_series(),)),)))
    base = ClassificationTask(input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("a",))
    derived = AnswerTask(
        input_modalities=frozenset({InputModality.TEXT, InputModality.TIME_SERIES}),
        inputs=(record,),
        prompt="q",
...
def test_add_tasks_rejects_duplicate_ids_within_the_batch(make_series):
    ...
    with pytest.raises(TimeFValidationError, match="share an id"):
        ds.add_tasks(
            tasks=[
                ClassificationTask(
                    input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("a",), id="dup"
                ),
                ClassificationTask(
                    input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("b",), id="dup"
                ),
            ]
        )
    assert ds.tasks == ()
...
def test_add_tasks_rejects_an_id_already_registered(make_series):
    ...
    ds.add_task(
        task=ClassificationTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=("a",), id="task-0"
        )
    )
    ...
    with pytest.raises(TimeFValidationError, match="not registered"):
        ds.add_task(
            task=TSCorrespondenceTask(
                input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=(unknown,)
            )
        )
...
def test_add_task_accepts_a_time_series_ref_on_the_record(make_series):
    ...
    task = ds.add_task(
        task=TSCorrespondenceTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=(signal,)
        )
    )
...
def test_add_task_accepts_mixed_targets(make_series):
    ...
    task = ds.add_task(
        task=TSCorrespondenceTask(
            input_modalities=frozenset({InputModality.TIME_SERIES}), inputs=(record,), targets=(record, signal, "match")
        )
    )
