...
def test_typed_scope_and_payload_columns_round_trip(tmp_path):
    ...
    with DuckDBControlReader(path) as reader:
        ...
        stored = (
            ...
            .execute("SELECT task_id, scope_type, target_schema, unit, target_name, mode FROM tasks ORDER BY task_id")
            .fetchall()
        )
    ...
    assert all(isinstance(row[1], str | type(None)) for row in stored)
    assert {row[0] for row in stored} == set(restored)
