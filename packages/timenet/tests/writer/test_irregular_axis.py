...
def test_a_mixed_record_keeps_each_series_on_its_own_axis(tmp_path):
    ...
    series = (
        Signal.from_irregular([21.3, 21.4, 21.9], time_offsets_us=shared_us, spec=_spec(), name="temp"),
        Signal.from_irregular([55.1, 55.3, 54.8], time_offsets_us=shared_us, spec=_spec(), name="humidity"),
        Signal.from_values(
        ...
        Signal.from_irregular(_HATCH_VALUES, time_offsets_us=_HATCH_US, spec=_spec("mach"), name="hatch"),
    )
    dataset.add_record(record=Record(sources=(Source(name="Source", signals=series),)))
    dataset.derive_schema()

    reader = TimeFReader(DatasetVersion.open_local(_written(tmp_path, dataset)))
    back = {ts.name: ts for ts in next(iter(reader.iter_records())).signals}
...
def test_time_offsets_must_match_the_value_count():
    with pytest.raises(TimeFValidationError, match="one time offset per value"):
        Signal.from_irregular([1.0, 2.0], time_offsets_us=[0, 1, 2], spec=_spec(), name="c")
...
def test_zarr_time_offsets_survive_a_tuned_chunk_size(tmp_path, chunk_max_bytes):
    ...
    back = next(iter(TimeFReader(DatasetVersion.open_local(version)).iter_records())).signals[0]
    assert back.time_offsets_us().tolist() == _HATCH_US
            ...
            sources=(
                Source(
                    ...
                    signals=(
                        ...
                        Signal.from_irregular(_HATCH_VALUES, time_offsets_us=_HATCH_US, spec=_spec(), name="hatch"),
                    ),
                ),
            ),
        )
...
def test_reader_rejects_time_offsets_disagreeing_with_the_stored_axis(tmp_path):
    ...
    with duckdb.connect(str(version_dir / "control.duckdb")) as connection:
        connection.execute("UPDATE axes SET last_us = last_us + 1000 WHERE axis_type = 'irregular'")
