...
def test_annotations_are_sorted_by_their_id_column(tmp_path):
    # Improvement B: the reader resolves an annotation by its id, so the file is sorted by id (not by
    # key) and its id column carries row-group statistics, which is what lets a lookup prune row groups.
    ann = pq.ParquetFile(_written(tmp_path) / "annotations.parquet")
    ids = [row["id"] for row in ann.read().to_pylist()]
    assert ids == sorted(ids)
    column = ann.metadata.schema.names.index("id")
    stats = ann.metadata.row_group(0).column(column).statistics
    assert stats is not None and stats.has_min_max
