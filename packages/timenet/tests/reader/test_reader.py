...
def _open(version_dir: Path) -> DatasetVersion:
    """Open a written version directory as the storage handle the reader now takes."""
    return DatasetVersion.open_local(version_dir)
...
def test_iter_samples_with_an_unknown_id_raises(tmp_path):
    version_dir = _write(tmp_path)
    with TimeFReader(_open(version_dir)) as reader, pytest.raises(TimeFValidationError, match="no such sample"):
...
def test_open_decodes_no_control_plane_table(tmp_path, monkeypatch):
    ...
    reader = TimeFReader(_open(version_dir))
...
def test_read_back_dataset_is_picklable(tmp_path, backend):
    ...
    dataset = TimeFReader(_open(version_dir)).read()
...
def test_reader_built_from_dataset_version_pickles(tmp_path):
    # The reader keeps only its handle (filesystem, root, manifest) plus dropped caches, so a reader
    # built from a DatasetVersion pickles even after a value read populated the backend's caches.
    version_dir = _write(tmp_path)
    reader = TimeFReader(_open(version_dir))
    original = next(iter(reader.iter_samples())).time_series[0].to_arrow()

    restored = pickle.loads(pickle.dumps(reader))
    assert restored._values is None  # caches dropped on pickle
    assert next(iter(restored.iter_samples())).time_series[0].to_arrow().equals(original)


# ---- validation -------------------------------------------------------------------------------


def test_open_local_missing_root_raises(tmp_path):
    # The reader trusts its handle; a missing version directory is caught building the handle instead.
    with pytest.raises(FileNotFoundError):
        DatasetVersion.open_local(tmp_path / "nope")


def test_open_local_missing_manifest_raises(tmp_path):
...
def test_missing_listed_file_fails_lazily_on_first_access(tmp_path):
    ...
    reader = TimeFReader(_open(version_dir))  # construction is happy
    with pytest.raises(FileNotFoundError):
...
def test_ordinal_row_carrying_regular_columns_raises_format_error(tmp_path):
    ...
    with TimeFReader(_open(version_dir)) as reader, pytest.raises(TimeFFormatError, match="carries regular- or"):
        list(reader.iter_samples())
...
def test_regular_row_with_a_zero_denominator_raises_format_error(tmp_path):
    ...
    with TimeFReader(_open(version_dir)) as reader, pytest.raises(TimeFFormatError, match="unbuildable regular axis"):
        list(reader.iter_samples())
...
def test_annotation_shape_disagreeing_with_its_descriptor_raises_format_error(tmp_path):
    ...
    with TimeFReader(_open(version_dir)) as reader, pytest.raises(TimeFFormatError, match="decodes to shape"):
        list(reader.iter_samples())
...
def test_annotation_value_type_disagreeing_with_its_descriptor_raises_format_error(tmp_path):
    ...
    with TimeFReader(_open(version_dir)) as reader, pytest.raises(TimeFFormatError, match="value type"):
        list(reader.iter_samples())
...
def test_annotation_span_outside_the_series_raises_format_error(tmp_path):
    ...
    with TimeFReader(_open(version_dir)) as reader, pytest.raises(TimeFFormatError, match="falls outside sample"):
        list(reader.iter_samples())
