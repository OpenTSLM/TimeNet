...
def _insert_records(connection, ids):
    for index, record_id in enumerate(ids):
        connection.execute(
            "INSERT INTO records VALUES (?, ?, NULL, NULL, NULL, [], [])", [record_id, f"record-{index}"]
        )
...
def test_a_chunk_naming_no_series_is_rejected(db):
    db.execute("INSERT INTO signal_chunks VALUES (3, 0, 0, 0, 0, 10)")
...
def test_a_chunk_naming_an_undeclared_artifact_is_rejected(db):
    db.execute("INSERT INTO signal_chunks VALUES (0, 0, 9, 0, 0, 10)")
...
def test_a_parquet_chunk_without_a_row_offset_is_rejected(db):
    ...
    db.execute("INSERT INTO signal_chunks VALUES (0, 0, 0, 0, NULL, 10)")
...
def test_a_zarr_chunk_with_a_row_offset_is_rejected(db):
    ...
    db.execute("INSERT INTO signal_chunks VALUES (0, 0, 0, 0, 4, 10)")
    assert _offenders(db, "zarr chunk with a row offset")


def _insert_series(connection, *, n_values):
    connection.execute("INSERT INTO specs VALUES (0, 's')")
...
def test_a_series_length_disagreeing_with_its_chunks_is_rejected(db):
    ...
    db.execute("INSERT INTO signal_chunks VALUES (0, 0, 0, 0, 0, 4)")
...
def test_a_series_length_matching_its_chunks_passes(db):
    _insert_series(db, n_values=10)
    db.execute("INSERT INTO signal_chunks VALUES (0, 0, 0, 0, 0, 4)")
    db.execute("INSERT INTO signal_chunks VALUES (0, 1, 0, 0, 1, 6)")
...
def test_a_from_task_item_is_not_checked(db):
    ...
    db.execute("INSERT INTO tasks VALUES (0, 'task-0', 'answer', NULL, NULL)")
    db.execute("INSERT INTO task_items VALUES (0, 'from_task', 0, 'missing')")
    assert _failing(db) == []
