...
from timenet.types import ForecastingTask, Task, Version
...
def remove_samples(dataset: TimeFDataset, sample_ids: Iterable[str], *, cascade: bool = False) -> TimeFDataset:
    ...
    edited = TimeFDataset.from_parts(
        metadata=dataset.metadata, samples=samples, tasks=tasks, schema=dataset.schema or dataset.derive_schema()
    )
...
def edit_version(
    ...
    **writer_kwargs: object,
    ...
    base_version = str(base.metadata.dataset_version)
    ...
    edited = TimeFDataset.from_parts(
        ...
        tasks=edited.tasks,
        schema=edited.schema or edited.derive_schema(),
    )
    edited.derive_schema()
