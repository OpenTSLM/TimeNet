...
class TimeFDataset:  # noqa: PLR0904
    ...
    @overload
    def add_task(self, task: TTask, /) -> TTask: ...
    ...
    def add_task(
        self,
        records: Record | Iterable[Record] | None,
        task: TTask,
    ) -> TTask: ...

    def add_task(
        self,
        records: Record | Iterable[Record] | Task | None = None,
        task: Task | None = None,
    ) -> Task:
        ...
        if isinstance(records, Task):
            raise TimeFValidationError("add_task accepts a positional task or task=, not both")
