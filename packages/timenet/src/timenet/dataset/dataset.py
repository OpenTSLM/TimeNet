...
class TimeFDataset:
    ...
    def __init__(self, *, metadata: DatasetMetadata) -> None:
        ...
        # The ids in _records, kept in step with it so a reference check needs no new set.
        self._record_ids: set[str] = set()
        self._tasks: list[Task] = []
        # The ids in _tasks, kept in step with it so an id check needs no new set.
    ...
    def _check_record_refs(self, task: Task) -> None:
        ...
        This runs for every task, including every streamed one. A task type that declares no
        record-id field returns at once.
