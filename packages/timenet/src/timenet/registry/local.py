...
class LocalRegistry(WritableRegistry):
    ...
    def open_version(self, dataset_id: str, version: str | None = None) -> DatasetVersion:
        ...
        Reuses the manifest :meth:`get_manifest` already parsed and validated (so a missing version or
        a misplaced artifact is caught there) and roots the handle at the version's directory over a
