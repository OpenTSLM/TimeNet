...
class DatasetVersion:
    ...
    """The filesystem the version's files live on (``LocalFileSystem`` now, ``S3FileSystem`` later)."""
    root: str
    ...
    def store_uri(self, relpath: str) -> str:
        ...
        instead, which is what this hands it. For a local version that is the plain filesystem path (Zarr
        opens it as a ``LocalStore``); an object-store version will return the URI its store library
        opens, which is why the two accessors are kept distinct even though they coincide for local.
