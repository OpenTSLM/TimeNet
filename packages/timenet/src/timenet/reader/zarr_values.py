...
def _to_arrow(values: np.ndarray, spec: TimeSeriesSpec) -> pa.Array:
    ...
    if spec.value_shape:
        return pa.FixedShapeTensorArray.from_numpy_ndarray(contiguous, dim_names=spec.dimension_names or None)
    return pa.array(contiguous, type=pa.from_numpy_dtype(np.dtype(spec.dtype)))
