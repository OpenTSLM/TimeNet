...
from timenet.types.trust import trusted_construction
...
class DuckDBControlReader:
    ...
    def task_table(self, record_ids: Iterable[str] | None = None) -> pa.Table:
        ...
        return self.connection.execute(
            f"""WITH scope_ids AS (
                    SELECT task_key, list(signal_id ORDER BY ordinal) AS scope_signal_ids
                    FROM (
                        SELECT task_key, unnest(scope_signal_keys) AS signal_key,
                               generate_subscripts(scope_signal_keys, 1) AS ordinal
                        FROM tasks
    ...
    def target_table(self, record_ids: Iterable[str] | None = None) -> pa.Table:
        ...
        return self.connection.execute(
            f"""WITH span_ids AS (
                    SELECT task_key, position, list(signal_id ORDER BY ordinal) AS span_signal_ids
                    FROM (
    ...
    def annotation_table(self, object_type: str | None = None) -> pa.Table:
        ...
        return self.connection.execute(
            f"""WITH object_ids AS ({object_ids}),
                span_ids AS (
                    SELECT occurrence_key, list(signal_id ORDER BY ordinal) AS signal_ids
                    FROM (
                        SELECT occurrence_key, unnest(signal_keys) AS signal_key,
                               generate_subscripts(signal_keys, 1) AS ordinal
    ...
    def _task_record_relationships(self, task_keys: list[int] | None) -> dict[int, dict[str, tuple[int, ...]]]:
        """Read ordered task-to-Record relationships for one batch, or every task, through integer keys.
        ...
        where, parameters = self._task_filter(task_keys)
        rows = self.connection.execute(
            f"SELECT task_key, field, record_key FROM task_record_refs {where} ORDER BY task_key, field, position",  # noqa: S608
            parameters,
        ).fetchall()
        return self._group_relationships(rows)

    def _task_annotation_relationships(self, task_keys: list[int] | None) -> dict[int, dict[str, tuple[int, ...]]]:
        """Read ordered task-to-annotation relationships for one batch, or every task, through integer keys.
        ...
        where, parameters = self._task_filter(task_keys)
        rows = self.connection.execute(
            f"SELECT task_key, field, occurrence_key FROM task_annotation_refs {where} ORDER BY task_key, field, position",  # noqa: S608
            parameters,
        ).fetchall()
        return self._group_relationships(rows)
    ...
    def _resolve_records(
        ...
        if not keys:
            return ()
        try:
            return tuple(map(records.__getitem__, keys))
    ...
    def _resolve_annotations(
        ...
        if not keys:
            return ()
        try:
            return tuple(map(annotations.__getitem__, keys))
        except KeyError as exc:
            raise TimeFFormatError(
...
class _TaskHydration:
    ...
    def _build(self, task_keys: list[int] | None) -> dict[int, Task]:  # noqa: PLR0914 - one batch joins several tables
        ...
        record_refs = reader._task_record_relationships(task_keys)
        annotation_refs = reader._task_annotation_relationships(task_keys)
        ...
        for row in task_rows:
            ...
            kwargs: dict[str, Any] = {
                "id": row.task_id,
                "inputs": reader._resolve_records(row.task_id, "inputs", refs, self.records_by_key),
            ...
            if "candidate_records" in cls.__dataclass_fields__:
                kwargs["candidate_records"] = reader._resolve_records(
                    row.task_id, "candidate_records", refs, self.records_by_key
                )
            task_annotation_refs = annotation_refs.get(row.task_key, {})
            kwargs["input_annotations"] = reader._resolve_annotations(
                row.task_id, "input_annotations", task_annotation_refs, annotations_by_occurrence
            )
            kwargs["target_annotations"] = reader._resolve_annotations(
                row.task_id, "target_annotations", task_annotation_refs, annotations_by_occurrence
            )
    ...
    def _signals_by_key(self, signal_keys: set[int]) -> dict[int, Signal]:
        ...
        for signal_key, signal_id in rows:
        ...
        if len(resolved) != len(signal_keys):
            raise TimeFFormatError(
                f"task target refers to missing signal key {sorted(signal_keys - resolved.keys())[0]!r}"
            )
        return resolved
