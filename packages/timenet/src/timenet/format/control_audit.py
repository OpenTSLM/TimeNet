...
from timenet.format.control_schema import CONTROL_TABLES


_UNIQUE_KEYS: tuple[tuple[str, tuple[str, ...]], ...] = tuple(
    (table.name, columns) for table in CONTROL_TABLES for columns in table.unique
)
"""Column sets that identify one row of their table, from the schema."""

_FOREIGN_KEYS: tuple[tuple[str, str, str, str], ...] = tuple(
    (table.name, key.column, key.table, key.references) for table in CONTROL_TABLES for key in table.foreign_keys
)
"""``(child table, child column, parent table, parent column)`` relationships, from the schema."""
