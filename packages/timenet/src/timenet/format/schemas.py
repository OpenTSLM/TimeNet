...
def index_schema(id_types: IdTypes) -> pa.Schema:
    ...
    return pa.schema(
        [
            ...
            ("channel", pa.string()),
            ("chunk_idx", pa.int32()),
            # Backend-neutral chunk locator. For Parquet, these are the shard path, row group,
            # and row offset. Future backends may assign different meanings to the coordinates.
            ("chunk_file", pa.string()),
            ("chunk_offset0", pa.int64()),
            ("chunk_offset1", pa.int64()),
            ("t_start_s", pa.float64()),
            ("t_end_s", pa.float64()),
            ("n_values", pa.int32()),
        ]
