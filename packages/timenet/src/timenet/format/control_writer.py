...
from timenet.format.control_schema import TABLES
...
_CONTENT_COLUMNS = TABLES["annotation_contents"].column_names
_OCCURRENCE_COLUMNS = TABLES["annotation_occurrences"].column_names
_TARGET_COLUMNS = TABLES["task_targets"].column_names
_TASK_COLUMNS = TABLES["tasks"].column_names
...
class _HierarchyBatches:
    ...
    def __init__(self, connection: duckdb.DuckDBPyConnection) -> None:
        self.records = _TableBatch(connection, "records", TABLES["records"].column_names)
        self.sources = _TableBatch(connection, "sources", TABLES["sources"].column_names)
        self.axes = _TableBatch(connection, "axes", TABLES["axes"].column_names)
        self.axis_offsets = _TableBatch(connection, "axis_offsets", TABLES["axis_offsets"].column_names)
        self.signals = _TableBatch(connection, "signals", TABLES["signals"].column_names)
...
class DuckDBControlWriter:
    ...
    def _write_chunks(
        ...
        chunks = _TableBatch(connection, "signal_chunks", TABLES["signal_chunks"].column_names)
    ...
    def _write_tasks(
        ...
        for batch in _task_batches(tasks_source):
            ...
            _insert_rows(connection, "tasks", _TASK_COLUMNS, task_rows)
            _insert_rows(connection, "task_targets", _TARGET_COLUMNS, target_rows)
            _insert_rows(connection, "task_record_refs", TABLES["task_record_refs"].column_names, record_ref_rows)
        ...
        _insert_rows(connection, "task_dependencies", TABLES["task_dependencies"].column_names, stored_dependencies)
    ...
    def _write_task_annotation_refs(
        ...
        for start in range(0, len(rows), _TASK_BATCH_SIZE):
            ...
            _insert_rows(connection, "task_annotation_refs", TABLES["task_annotation_refs"].column_names, stored)
    ...
    def _write_annotations(
        ...
        occurrence_keys: dict[str, int] = {}
        content_batch = _TableBatch(connection, "annotation_contents", _CONTENT_COLUMNS)
        occurrence_batch = _TableBatch(connection, "annotation_occurrences", _OCCURRENCE_COLUMNS)
