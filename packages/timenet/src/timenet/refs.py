...
def split_ref(ref: str) -> tuple[str, str | None]:
    ...
    Raises:
        ValueError: If ``ref`` has more than one ``@``, an empty id or version, or a version part that is
            neither ``latest`` nor a valid ``major.minor.patch``.
    """
    if ref.count("@") > 1:
        raise ValueError(f"dataset ref must contain at most one '@', got {ref!r}")
    dataset_id, separator, version = ref.partition("@")
    if not dataset_id:
        raise ValueError(f"dataset ref has an empty id: {ref!r}")
    if not separator or version == "latest":
        return dataset_id, None
    if not version:
        raise ValueError(f"dataset ref has an empty version: {ref!r}")
    Version.parse(version)  # validate the pin; raises ValueError on a malformed version
