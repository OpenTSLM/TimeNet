...
Builders construct them directly or subclass them with field defaults for reuse.
:class:`~timenet.control_plane.TimeFReader` rebuilds the identical instances from the manifest, so they
round-trip and pickle without any runtime class synthesis. The per-signal identifier lives on
:class:`~timenet.control_plane.model.Signal`, not here, so every signal of a modality shares one spec.
"""
