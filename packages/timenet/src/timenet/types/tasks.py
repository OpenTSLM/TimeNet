...
def _sync_ref_ids(task: "Task", object_field: str, id_field: str, *, empty_is_missing: bool = False) -> None:
    """Project one object-reference field into its temporary compatibility ID field.

    Raises:
        TimeFValidationError: If the object and ID forms disagree.
    """
    objects = getattr(task, object_field)
    if objects is None or (empty_is_missing and not objects):
        return
    projected = (
        tuple(cast("Record | Signal", value).id for value in objects)
        if isinstance(objects, tuple)
        else cast("Record | Signal", objects).id
    )
    stored = getattr(task, id_field)
    if stored is not None and not (empty_is_missing and not stored) and stored != projected:
        raise TimeFValidationError(f"{type(task).__name__} {object_field} and {id_field} disagree")
    setattr(task, id_field, projected)
    ...
    def __post_init__(self) -> None:
        ...
        Raises:
            TimeFValidationError: If the task sets neither ``target_record_id`` nor ``target_span``.
                ...
                with its own ``scope``.
        """
        _sync_ref_ids(self, "context_records", "context_record_ids", empty_is_missing=True)
        _sync_ref_ids(self, "target_record", "target_record_id")
...
class TSEditingTask(Task):
    ...
    def __post_init__(self) -> None:
        ...
        """
        _sync_ref_ids(self, "source_record", "source_record_id")
        _sync_ref_ids(self, "target_record", "target_record_id")
...
class TSCorrespondenceTask(Task):
    ...
    def __post_init__(self) -> None:
        ...
        """
        _sync_ref_ids(self, "candidate_records", "candidate_record_ids", empty_is_missing=True)
        _sync_ref_ids(self, "target_records", "target")
        _sync_ref_ids(self, "target_signals", "target_time_series_ids")
