...
resolution change with magnitude and two equal instants need not compare equal, which is not
...
def seconds_to_us(seconds: float) -> int:
    ...
    Args:
        seconds: An instant or duration in seconds.
...
def us_to_seconds(microseconds: int) -> float:
    ...
    Args:
        microseconds: An instant or duration in microseconds.
...
def unix_us(moment: datetime | int) -> int:
    """Normalize a wall-clock instant to Unix microseconds.
    ...
    delta = moment - datetime.fromtimestamp(0, tz=moment.tzinfo)
    return delta.days * 86_400 * US_PER_S + delta.seconds * US_PER_S + delta.microseconds
