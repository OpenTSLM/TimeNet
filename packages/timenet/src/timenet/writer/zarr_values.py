...
backend-agnostic ``chunk_file`` / ``chunk_offset0`` locator (``chunk_offset1`` is unused).
...
from timenet.writer.values import ChunkPlacement, ValuesWriteResult, ZarrValuesConfig
...
def _array_name(spec_type: str) -> str:
    """Encode a logical spec type as one collision-free Zarr path segment.
