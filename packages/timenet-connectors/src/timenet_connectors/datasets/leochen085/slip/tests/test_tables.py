...
@pytest.mark.parametrize("freq", ["0 sec", "0 min", "0.0 h"])
def test_a_period_of_zero_raises_rather_than_passing_for_no_rate(freq: str) -> None:
    # A zero period is falsy, so an unguarded one would read as a corpus that states no rate.
    with pytest.raises(TimeFFormatError, match="above zero"):
        parse_period_us(freq)
...
def test_a_period_finer_than_a_microsecond_raises_here_naming_the_cell(freq: str) -> None:
    ...
    with pytest.raises(TimeFFormatError, match="one microsecond"):
        parse_period_us(freq)
    # The bound is on the computed period, so it has to hold for a rate and for a period alike.
    # One microsecond is the finest the format carries, written either way, and both still parse.
...
def test_a_period_past_the_int64_ceiling_raises_here_too(freq: str) -> None:
    # The quantity pattern bounds neither end, so a period can overshoot as well as undershoot. Both
    # ends are caught here for the same reason: the axis names no cell.
...
def test_a_period_inside_the_range_whose_numerator_is_not_raises_here_too() -> None:
    ...
    # 1e19, which is not. Without the second check it would reach RegularAxis, which names no cell.
