...
ANGULAR_RATE = TimeSeriesSpec(
    spec_type="slip_eval_angular_rate",
    name="SLIP-eval Angular Rate",
    unit_value=ureg.radian / ureg.second,
    dtype=_DTYPE,
)
"""``uci_har``'s three gyroscope signals."""
...
"""The spec every signal of a folder uses, unless :func:`spec_for` narrows it."""


def spec_for(folder: str, signal: str) -> TimeSeriesSpec:
    """Give the spec one signal of one folder uses.

    Args:
        folder: The folder's directory name.
        signal: The signal's name, as ``folders.py`` states it.

    Returns:
        :data:`ANGULAR_RATE` for a gyroscope signal, and the folder's own spec otherwise.
    """
    return ANGULAR_RATE if signal.startswith("gyro") else BY_FOLDER[folder]
