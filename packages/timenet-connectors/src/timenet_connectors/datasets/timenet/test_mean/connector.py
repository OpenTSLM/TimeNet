...
so the dataset is fully deterministic and needs no network. It has no raw source, so it implements only
``convert``; it backs the end-to-end training example.
"""
...
class TestMeanConnector(BaseConnector[None]):
    ...
    Synthetic, so it needs no ``download``: :meth:`convert` generates every sample directly.
    """
