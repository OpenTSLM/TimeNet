"""Read HEARTS payloads with a restricted unpickler and a two-file cache.

The converter reads each case to build records and tasks. Lazy signal loaders
read it again when the writer needs values. The unpickler allows only known
pandas and NumPy globals; the small cache bounds memory use.
"""
