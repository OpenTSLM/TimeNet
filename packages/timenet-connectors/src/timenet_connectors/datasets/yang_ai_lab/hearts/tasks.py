"""Declarative task mappings for all thirty released HEARTS directories.

Each directory maps to the task type that matches the answer the reference harness scores: a
...
The prompts are adapted from ``exp/<source>/<task>.py`` in the release. File
references name TimeF signals instead. Case-specific values appear in input
annotations. The conversion choices are recorded in ``connector-review.md``.
...
class TaskDefinition:
    ...
    """Top-level payload fields that contain agent-visible series; None uses the source default."""
    no_record: bool = False
    """Whether the case supplies annotations and prompt without a record."""
    images: bool = False
    """Whether the case supplies meal photographs as image signals."""

    @property
    def input_modalities(self) -> frozenset[InputModality]:
        """Return the input kinds this directory gives the model.
...
_SYMPTOMS_ONLY_PROMPT = "Classify COVID status as healthy or covid_positive using only the supplied symptoms."
...
def _plain_scalar(value: np.generic) -> Any:
    """Convert one NumPy scalar into its exact Python value.
...
def build_task(  # noqa: PLR0913, PLR0917 - source case data and target must be explicit
    ...
    Args:
        ...
        id_prefix: The connector's id prefix.
        case_id: The stable source case identifier.
        target_record: A held-out record for forecasting and imputation.
