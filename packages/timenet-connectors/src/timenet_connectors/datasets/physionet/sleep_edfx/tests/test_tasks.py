...
def test_an_epoch_far_into_the_night_sits_where_its_entry_states():
    built = list(tasks.build_epoch_tasks(_SAMPLE_ID, _PREFIX, [_stage("Sleep stage W", 52260, 52290)]))
...
def test_an_epoch_is_placed_from_the_onset_it_is_given():
    ...
    built = tasks.build_epoch_tasks(_SAMPLE_ID, _PREFIX, [_stage("Sleep stage W", 0, 60)])
...
def test_a_second_build_of_one_scoring_gives_the_same_epochs():
    ...
    first = [_scope(one).start_us for one in tasks.build_epoch_tasks(_SAMPLE_ID, _PREFIX, stages)]
    second = [_scope(one).start_us for one in tasks.build_epoch_tasks(_SAMPLE_ID, _PREFIX, stages)]
...
def test_an_epoch_survives_a_change_to_an_entry_before_it():
    ...
    assert [_scope(one).start_us for one in tasks.build_epoch_tasks(_SAMPLE_ID, _PREFIX, before)] == [
        _scope(one).start_us for one in tasks.build_epoch_tasks(_SAMPLE_ID, _PREFIX, after)
    ]


def _fact(key, value, at_s=None):
    span = None if at_s is None else TimePoint.micros(at_s * US_PER_S)
    return Annotation(key=key, value=value, span=span, id=f"{_SAMPLE_ID}-{key}")
...
def test_the_lights_off_moment_becomes_a_localization_task():
    built = tasks.build_lights_off_task(_SAMPLE_ID, _SESSION, [_fact(AnnotationKey.LIGHTS_OFF, "22:30:00", 1800)])
...
def test_a_moment_the_session_does_not_hold_gets_no_task():
    ...
    assert (
        tasks.build_lights_off_task(_SAMPLE_ID, session, [_fact(AnnotationKey.LIGHTS_OFF, "22:30:00", 86370)]) is None
    )
...
def test_a_moment_at_the_last_instant_of_the_session_is_held():
    ...
    assert (
        tasks.build_lights_off_task(_SAMPLE_ID, session, [_fact(AnnotationKey.LIGHTS_OFF, "22:30:00", 1800)])
        is not None
    )
...
def test_a_sample_with_no_declared_session_gets_no_lights_off_task():
    assert tasks.build_lights_off_task(_SAMPLE_ID, None, [_fact(AnnotationKey.LIGHTS_OFF, "22:30:00", 1800)]) is None
