...
- Set up the environment with `make sync` (`uv sync --all-groups --all-extras --no-extra torch-cpu`),
  which installs every workspace member plus their optional extras (`timenet[cli,torch-gpu]`,
