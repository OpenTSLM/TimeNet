...
3. **Pinned Hub sources.** Share helpers that list and download files from a
   fixed Hugging Face dataset revision.
4. **Image signals.** Convert image files or bytes to RGB tensor signals with
   an ordinal axis. Keep this image conversion in its own commit.
5. **SLIP training.** Map the training Parquet shards to sensor records and
   four caption tasks per row. Keep source metadata and lazy signal loaders.
6. **SLIP evaluation.** Add its own registry card and connector. Reuse the
   source pin and nested-Parquet reader, retaining all eleven components and
   their native splits and labels.
7. **ARFBench.** Add shared metric/resolution records, tasks at every common
   resolution, and chart-input tasks with Zarr images.
8. **VerbalTS.** Pin all Drive files, verify them, and convert the six
   components with memory-mapped arrays and explicit prompt-only modalities.
9. **HEARTS source handling.** Add its restricted pickle reader and input
    signal adapters, including decoded photographs.
10. **HEARTS task map.** Declare all 30 task directories, their visible inputs,
    prompts, and answer types in one table.
11. **HEARTS integration.** Download the pinned release, create held-out
    sequence targets, and support recordless symptoms.
12. **Integration and documentation.** Run core and isolated connector checks;
